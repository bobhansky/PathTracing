...
struct Thread_arg {
	...
	const Vector3d* delta_h;
	const Vector3d* c_off_h;
	const Vector3d* c_off_v;
...
public:
	...
	/// <returns></returns>
	Vector3d calcForMirror(const Vector3d& origin, const Vector3d& dir, Intersection& inter, int depth) {
		...
		Vector3d rayOrig = inter.pos + inter.nDir * EPSILON;
		interStrategy->UpdateInter(x_inter, g->scene, rayOrig, wi);
		if (x_inter.intersected) {
			double cos = inter.nDir.dot(wi);
			Vector3d wo = -dir;
			double pdf = inter.mtlcolor.pdf(wo, wi, inter.nDir);
			Vector3d f_r = inter.mtlcolor.BxDF(wi, wo, inter.nDir, g->eta);

			Vector3d res = traceRay(rayOrig, wi, depth + 1, Vector3d(1));
	...
	/// <returns></returns>
	Vector3d calcForRefractive(const Vector3d& origin, const Vector3d& dir, Intersection& inter, int depth,
		int thdID = -1, bool recording = false) {
		if (depth > MAX_DEPTH) return 0;
		Vector3d N = inter.nDir;

		Vector3d wo = -dir;
		double eta_i = g->eta;
		double eta_t = inter.mtlcolor.eta;

		Vector3d wi;
		auto [sampleSuccess, TIR] = inter.mtlcolor.sampleDirection(wo, N, wi, eta_i);
		wi = normalized(wi);
		double pdf = inter.mtlcolor.pdf(wi, wo, N, eta_i, eta_t);
		...
		if (TIR) {
			...
			if (inter.mtlcolor.mType == MICROFACET_T) {
				...
				pdf = 1 * D_ndf(h, interN, inter.mtlcolor.roughness) * cosTheta / (4.0 * wo.dot(h));
			}
		}
		Vector3d f_r = inter.mtlcolor.BxDF(wi, wo, N, eta_i, TIR);

		Vector3d rayOrig = inter.pos;
		double cos = 0;
		...
		Vector3d Li = traceRay(rayOrig, wi, depth + 1, Vector3d(1));
	...
	Vector3d traceRay(const Vector3d& origin, const Vector3d& dir, int depth, Vector3d tp,
		...
		double light_pdf;
		double mis_weight_l = 0.0;
		double mat_pdf;
		double mis_weight_m = 0.0;
		Intersection light_inter;
		sampleLight(light_inter, light_pdf, g);

		bool rayInside = inter.nDir.dot(wo) < 0;
		Vector3d shadowRayOrig = inter.pos;
		...
		if (!light_inter.intersected || isShadowRayBlocked(shadowRayOrig, light_inter.pos, g)) {}
			...
			Vector3d wi = light_inter.pos - inter.pos;
			double r2 = wi.norm2();
			wi = normalized(wi);
			if (wi.dot(light_inter.nDir) > 0) {}
			else {
				mat_pdf = inter.mtlcolor.pdf(wi, wo, inter.nDir, g->eta, inter.mtlcolor.eta);	// w.r.t solid angle
				Vector3d light_N = normalized(light_inter.nDir);
				double cos_theta_prime = light_N.dot(-wi);
				if (cos_theta_prime <= 0) goto jmp;
				double dot = inter.nDir.dot(wi);
				double cos_theta = dot < 0 ? 0 : dot;
	...
	jmp:
		...
		if (!x_inter.intersected) {}
			...
			double dot = abs(inter.nDir.dot(wi));
			double cos_theta = dot;

			light_pdf = getLightPdf(x_inter, g);
			if (light_pdf) {
				Vector3d light_N = normalized(x_inter.nDir);
				double cos_theta_prime = light_N.dot(-wi);
				...
				// pdf_l = pdf_m * cos_theta_prime / r2
				double r2 = (x_inter.pos - inter.pos).norm2();
				double l_pdf_transformed = light_pdf * r2 / cos_theta_prime;

				mis_weight_m = getMisWeight(mat_pdf, l_pdf_transformed);
				if (inter.mtlcolor.mType == PERFECT_REFLECTIVE && mat_pdf == 1.0)
					mis_weight_m = 1.0;
				Vector3d f_r = inter.mtlcolor.BxDF(wi, wo, inter.nDir, g->eta);
				Vector3d L_i = x_inter.mtlcolor.emission;
			...
			jmp2:
				...
				double rr_prob = std::max(tp.x, std::max(tp.y, tp.z));
				if (getRandomFloat() > rr_prob)
					return sampleValue;

				Vector3d f_r = inter.mtlcolor.BxDF(wi, wo, inter.nDir, g->eta);
				Vector3d coe = f_r * cos_theta / (mat_pdf * rr_prob);
				if (mat_pdf * rr_prob < MIN_DIVISOR) return sampleValue;
				tp = tp * coe;

				Vector3d Li = traceRay(rayOrig, wi, depth + 1, tp, &x_inter, thdID, recording);
		...
		Vector3f dir_illu(0.0);
		Vector3f indir_illu(0.0);
		// ****** Direct illumination
		double light_pdf;
		Intersection light_inter;
		sampleLight(light_inter, light_pdf, g);
		if (!light_inter.intersected) {}
			...
			if (isShadowRayBlocked(shadowRayOrig, light_inter.pos, g)) {}
				...
				double cos_theta_prime = light_N.dot(-p_to_light);
				// if light does not illuminate this direction (p_to_light is on the back side of the light)
				if (cos_theta_prime < 0) {}
				else {
					double dis2 = (light_inter.pos - inter.pos).norm2();
					double cos_theta = p_to_light.dot(inter.nDir);
		...
		if (x_inter.intersected && !x_inter.mtlcolor.hasEmission()) {
			double pdf = inter.mtlcolor.pdf(wi, wo, inter.nDir, g->eta, inter.mtlcolor.eta);
			double cos_theta = abs(inter.nDir.dot(wi));
	...
	virtual void integrate(PPMGenerator* g) {
		...
		if (g->parallel_projection) d = 4.0;
...
void sub_render(Thread_arg* a, int threadID, int s, int e) {
	...
	const Vector3d ul = *arg.ul;
	const Vector3d delta_v = *arg.delta_v;
	const Vector3d delta_h = *arg.delta_h;
	const Vector3d c_off_h = *arg.c_off_h;
	const Vector3d c_off_v = *arg.c_off_v;
	PPMGenerator* g = arg.g;
	PathTracing* pt = arg.pt;
	std::vector<Vector3d>* rgb_array = &(g->cam.FrameBuffer.rgb);
	const Vector3d eyePos = *arg.eyePos;
	int Ncol = g->width;

	for (int y = s; y < e; y++) {
		for (int x = 0; x < Ncol; x++) {
			Vector3d& color = rgb_array->at(g->getIndex(x, y));
			Vector3d rayDir = { 0,0,0 };
			Vector3d pixelPos = ul + x * delta_h + y * delta_v + c_off_v + c_off_v;
			rayDir = normalized((pixelPos - eyePos));

			// trace ray into each pixel
			Vector3d estimate;
			for (int i = 0; i < SPP; i++) {
				Vector3d res = pt->traceRay(eyePos, rayDir, 0, Vector3d(1), nullptr, threadID, RECORD);
